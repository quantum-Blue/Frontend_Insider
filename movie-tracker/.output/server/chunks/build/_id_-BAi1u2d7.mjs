...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    const { data: tvShow, pending: tvPending, error: tvError } = ([__temp, __restore] = withAsyncContext(() => useFetch(`/tv/${route.params.id}`, {
      baseURL: config.public.tmdbBaseUrl,
      params: {
        api_key: config.public.tmdbApiKey
      }
    }, "$Umbr3JvyKL")), __temp = await __temp, __restore(), __temp);
    const { data: credits, pending: creditsPending } = ([__temp, __restore] = withAsyncContext(() => useFetch(`/tv/${route.params.id}/credits`, {
      baseURL: config.public.tmdbBaseUrl,
      params: {
        api_key: config.public.tmdbApiKey
      }
    }, "$smqoS3L-_n")), __temp = await __temp, __restore(), __temp);
    const { data: similar, pending: similarPending } = ([__temp, __restore] = withAsyncContext(() => useFetch(`/tv/${route.params.id}/similar`, {
      baseURL: config.public.tmdbBaseUrl,
      params: {
        api_key: config.public.tmdbApiKey,
        language: "en-US",
    ...
    }, "$06CGlUZiKa")), __temp = await __temp, __restore(), __temp);
    ...
    useSeoMeta({
      ...
      ogImage: () => tvShow.value?.backdrop_path ? `https://image.tmdb.org/t/p/w1280${tvShow.value.backdrop_path}` : void 0,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (pending.value) {
        ...
        _push(`</div></div></div>`);
      } else if (unref(tvShow)) {
        _push(`<div class="min-h-screen bg-gray-50">`);
        if (unref(tvShow).backdrop_path) {
          _push(`<div class="relative h-96 lg:h-[500px] bg-cover bg-center" style="${ssrRenderStyle({ backgroundImage: `url(https://image.tmdb.org/t/p/w1280${unref(tvShow).backdrop_path})` })}"><div class="absolute inset-0 bg-black/50"></div><div class="absolute inset-0 bg-gradient-to-t from-gray-50 via-transparent to-transparent"></div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<div class="container mx-auto px-4"><div class="relative -mt-32 lg:-mt-40 pb-12"><div class="flex flex-col lg:flex-row gap-8"><div class="lg:w-1/3 flex-shrink-0"><div class="relative"><img${ssrRenderAttr("src", posterUrl.value)}${ssrRenderAttr("alt", unref(tvShow).name)} class="w-full max-w-sm mx-auto lg:mx-0 rounded-xl shadow-2xl"><button class="${ssrRenderClass([isFavorite.value ? "bg-red-500/90 text-white hover:bg-red-600/90" : "bg-black/50 text-white hover:bg-black/70", "absolute top-4 right-4 p-3 rounded-full backdrop-blur-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-white/50 shadow-lg"])}"><svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">`);
        if (isFavorite.value) {
        ...
        _push(`</svg></button></div></div><div class="lg:w-2/3 bg-white rounded-xl shadow-xl p-8"><div class="mb-6"><h1 class="text-4xl lg:text-5xl font-bold text-gray-900 mb-2">${ssrInterpolate(unref(tvShow).name)}</h1>`);
        if (unref(tvShow).tagline) {
          _push(`<p class="text-xl text-gray-600 italic">${ssrInterpolate(unref(tvShow).tagline)}</p>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div><div class="flex flex-wrap items-center gap-6 mb-6"><div class="flex items-center gap-2"><svg class="w-6 h-6 text-yellow-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg><span class="text-xl font-semibold text-gray-900">${ssrInterpolate(unref(tvShow).vote_average?.toFixed(1) || "N/A")}</span><span class="text-gray-500">/10</span></div><div class="flex items-center gap-2 text-gray-600"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg><span class="font-medium">${ssrInterpolate(formatAirDate(unref(tvShow).first_air_date))}</span></div>`);
        if (unref(tvShow).number_of_seasons) {
          _push(`<div class="flex items-center gap-2 text-gray-600"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 4V2a1 1 0 011-1h8a1 1 0 011 1v2h4a1 1 0 011 1v2a1 1 0 01-1 1h-1v12a2 2 0 01-2 2H6a2 2 0 01-2-2V8H3a1 1 0 01-1-1V5a1 1 0 011-1h4z"></path></svg><span class="font-medium">${ssrInterpolate(unref(tvShow).number_of_seasons)} Season${ssrInterpolate(unref(tvShow).number_of_seasons !== 1 ? "s" : "")}</span></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div><div class="flex flex-wrap items-center gap-4 mb-6">`);
        if (unref(tvShow).status) {
          _push(`<span class="${ssrRenderClass([getStatusClass(unref(tvShow).status), "px-3 py-1 rounded-full text-sm font-medium"])}">${ssrInterpolate(unref(tvShow).status)}</span>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(tvShow).last_air_date) {
          _push(`<span class="text-gray-600 text-sm"> Last aired: ${ssrInterpolate(formatAirDate(unref(tvShow).last_air_date))}</span>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(tvShow).number_of_episodes) {
          _push(`<span class="text-gray-600 text-sm">${ssrInterpolate(unref(tvShow).number_of_episodes)} episodes </span>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
        if (unref(tvShow).genres?.length) {
          _push(`<div class="mb-6"><div class="flex flex-wrap gap-2"><!--[-->`);
          ssrRenderList(unref(tvShow).genres, (genre) => {
        ...
        if (unref(tvShow).overview) {
          _push(`<div class="mb-8"><h2 class="text-2xl font-bold text-gray-900 mb-3">Overview</h2><p class="text-gray-700 leading-relaxed text-lg">${ssrInterpolate(unref(tvShow).overview)}</p></div>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(tvShow).networks?.length) {
          _push(`<div class="mb-6"><h3 class="text-lg font-semibold text-gray-900 mb-3">Networks</h3><div class="flex flex-wrap gap-3"><!--[-->`);
          ssrRenderList(unref(tvShow).networks, (network) => {
            _push(`<div class="flex items-center gap-2 bg-gray-100 px-3 py-2 rounded-lg">`);
            if (network.logo_path) {
              _push(`<img${ssrRenderAttr("src", `https://image.tmdb.org/t/p/w92${network.logo_path}`)}${ssrRenderAttr("alt", network.name)} class="h-6 object-contain">`);
            } else {
              _push(`<!---->`);
            }
            _push(`<span class="text-sm font-medium text-gray-800">${ssrInterpolate(network.name)}</span></div>`);
          });
          _push(`<!--]--></div></div>`);
        } else {
          _push(`<!---->`);
        ...
        if (unref(tvShow).original_language) {
          _push(`<div class="flex justify-between py-2 border-b border-gray-200"><span class="font-semibold text-gray-600">Language:</span><span class="text-gray-900">${ssrInterpolate(unref(tvShow).original_language.toUpperCase())}</span></div>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(tvShow).type) {
          _push(`<div class="flex justify-between py-2 border-b border-gray-200"><span class="font-semibold text-gray-600">Type:</span><span class="text-gray-900">${ssrInterpolate(unref(tvShow).type)}</span></div>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(tvShow).episode_run_time?.length) {
          _push(`<div class="flex justify-between py-2 border-b border-gray-200"><span class="font-semibold text-gray-600">Episode Runtime:</span><span class="text-gray-900">${ssrInterpolate(unref(tvShow).episode_run_time[0])} min</span></div>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(tvShow).in_production !== void 0) {
          _push(`<div class="flex justify-between py-2 border-b border-gray-200"><span class="font-semibold text-gray-600">In Production:</span><span class="text-gray-900">${ssrInterpolate(unref(tvShow).in_production ? "Yes" : "No")}</span></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div></div></div>`);
        ...
        if (unref(similar)?.results?.length) {
          _push(`<div class="pb-12"><div class="bg-white rounded-xl shadow-xl p-8"><h2 class="text-3xl font-bold text-gray-900 mb-8">Similar TV Shows</h2><div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6"><!--[-->`);
          ssrRenderList(unref(similar).results.slice(0, 6), (similarShow) => {
            _push(ssrRenderComponent(_component_NuxtLink, {
              key: similarShow.id,
              ...
              default: withCtx((_, _push2, _parent2, _scopeId) => {
                if (_push2) {
                  _push2(`<div class="relative mb-3"${_scopeId}>`);
                  if (similarShow.poster_path) {
                    _push2(`<img${ssrRenderAttr("src", `https://image.tmdb.org/t/p/w342${similarShow.poster_path}`)}${ssrRenderAttr("alt", similarShow.name)} class="w-full aspect-[2/3] object-cover rounded-lg shadow-md group-hover:shadow-lg transition-shadow"${_scopeId}>`);
                  } else {
                  ...
                  _push2(`</div><h3 class="font-semibold text-gray-900 text-sm mb-1 group-hover:text-blue-600 transition-colors"${_scopeId}>${ssrInterpolate(similarShow.name)}</h3>`);
                  if (similarShow.first_air_date) {
                    _push2(`<p class="text-gray-600 text-xs"${_scopeId}>${ssrInterpolate(new Date(similarShow.first_air_date).getFullYear())}</p>`);
                  } else {
                    _push2(`<!---->`);
                  }
                } else {
                  return [
                    createVNode("div", { class: "relative mb-3" }, [
                      similarShow.poster_path ? (openBlock(), createBlock("img", {
                        key: 0,
                    ...
                    createVNode("h3", { class: "font-semibold text-gray-900 text-sm mb-1 group-hover:text-blue-600 transition-colors" }, toDisplayString(similarShow.name), 1),
                    similarShow.first_air_date ? (openBlock(), createBlock("p", {
                      key: 0,
                      class: "text-gray-600 text-xs"
                    }, toDisplayString(new Date(similarShow.first_air_date).getFullYear()), 1)) : createCommentVNode("", true)
                  ];
                }
              }),
              _: 2
...
//# sourceMappingURL=_id_-BAi1u2d7.mjs.map
